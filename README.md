...
1. git clone https://github.com/JJM258/applied_statistical_modelling_project.git
2. Open Stat_Project.R in RStudio
3. Install required packages if not already installed:
4. r
   - install.packages("tidyverse")
   - install.packages("caret")
5. Ensure amazon_ecommerce_1M.csv is in the same working directory as the script (use getwd() to check, setwd() to change if needed)
6. Run the script in order, from data ingestion through to data visualisation
